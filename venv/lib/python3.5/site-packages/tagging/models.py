...
class TagManager(models.Manager):
    ...
    def usage_for_model(self, model, counts=False, min_count=None,
                        filters=None):
        """
        주어진 모델 클래스의 인스턴스와 관련된 태그 리스트를 얻습니다.
...
class TaggedItemManager(models.Manager):
    ...
    FIXME There's currently no way to get the ``GROUP BY`` and ``HAVING``
          ...
          지금, 우리는 수동으로 쿼리를 실행하여 관심있는 객체의 pk를 검색한 후 
          ORM 의 "__in" 검색을 사용하여 "QerySet"을 반환한다.
    ...
    def get_by_model(self, queryset_or_model, tags):
        """
        주어진 태그 또는 태그 목록과 관련된 특정 모델의 인스턴스를 포함하는 QuerySet을 만듭니다. 
        """
        tags = get_tag_list(tags)
        tag_count = len(tags)
        if tag_count == 0:
    ...
    def get_intersection_by_model(self, queryset_or_model, tags):
        """
        주어진 태그 목록의 *모든 것과* 관련있는 특정 모델의 인스턴스를 포함한 QuerySet을 만든다.
        """
        tags = get_tag_list(tags)
        tag_count = len(tags)
        queryset, model = get_queryset_and_model(queryset_or_model)
